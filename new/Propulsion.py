...
class Propulsion(ABC):
    ...
    Having them all under one interface makes it easier to write reusable code.
    """
    @abstractmethod
    ...
    def travel_towards(self, heading):
        ...
        For tank drive, face(heading, maximumOutput) should be called to accomplish that.
    ...
    def correct_for(self, error_vector):
        """
        Should correct for positional error during autonomous using some combination of the other methods and PID
    ...
    def complete_loop_update(self):
        ...
        For example, swerve must be given a heading from travel_towards(), a speed from set_speed(), and a spin from set_spin() before holonomic computations can be performed.
